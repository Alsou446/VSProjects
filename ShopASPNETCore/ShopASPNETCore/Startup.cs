...
namespace ShopASPNETCore
{
    public class Startup
    {
        ...
        public Startup(IWebHostEnvironment hostEnv) //конструктор подключения к 
        {
            _confString = new ConfigurationBuilder().SetBasePath(hostEnv.ContentRootPath).AddJsonFile("dbsetings.json").Build();//получение строки из файла
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            app.UseMvc(routes => 
            {
                ...
                routes.MapRoute(name: "curTasksList", template: "Tasks/{action}/{TasksList}", defaults: new { Controller="Tasks", acrion= "ResultList" });
            });
